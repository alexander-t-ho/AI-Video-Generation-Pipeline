 ...
 * Recolors a vehicle image using google/nana-banana-pro model
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Choose model based on image source - nana-banana-pro requires S3 URLs
    if (isS3Url(body.imageUrl)) {
      // Use google/nano-banana model for S3-hosted images (display name: Nana Banana Pro)
      setRuntimeImageModel('google/nano-banana');
      console.log(`[Recolor API] Using nana-banana-pro model for S3 URL`);
    } else {
    ...
    // Generate recolored image using nana-banana-pro model
    // We pass the color reference URL in the referenceImageUrls array
    // The image generator will handle adding it to image_input for nana-banana-pro
