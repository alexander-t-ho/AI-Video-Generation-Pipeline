...
export default function BackgroundImageUpload({
  ...
  const handleFilesSelected = async (files: File[]) => {
    ...
    try {
      ...
      console.error('[BackgroundImageUpload] Error uploading image:', error);
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-gray-500">(should already be in directing style)</span>
      </div>
          ...
          <div className="relative w-full h-64 rounded-lg overflow-hidden border-2 border-gray-300 bg-gray-100">
            {imageLoadError && previewBlobUrl ? (
              ...
              <img
                ...
                className="w-full h-full object-cover"
                onError={() => {
                  console.warn('[BackgroundImageUpload] Failed to load image, trying blob fallback');
