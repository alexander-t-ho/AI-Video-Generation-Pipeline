...
async function main() {
  ...
  try {
    ...
    } else if (error.message.includes('402') || error.message.includes('Payment Required') || error.message.includes('credit')) {
      console.log('\n💳 Payment/Credit Error:');
      console.log('   - Your account has insufficient credits');
      console.log('   - Add credits at: https://replicate.com/account/billing#billing');
      console.log('   - Wait a few minutes after adding credits before trying again');
    } else {
      console.log('\n🔧 Other Error:');
